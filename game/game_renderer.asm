...
render_grid_objects:
    move $s4, $v0                           # load the grid into a saved temporary into $s4
    li $s0, 16                              # $s0 = number of rows in game grid
    li $s1, 8                               # $s1 = number of columns in game grid
    
    li $s3, 0                               # $s3 = current row index
    # Outer loop - iterate over rows
    read_grid_outer_loop:
        beq $s3, $s0, return_ren            # return if already did last row ($s3 == $s0)

        # Inner loop - iterate over columns
        add $s2, $zero, $zero               # $s2 = current column index
        read_grid_inner_loop:
            beq $s2, $s1, end_read_grid_inner_loop        # return if already did last column ($s2 == $s1)
            ...
            lh $t5, 0($t4)                      # $t5 = element at $t4
            
            # Colour the pixels in the display
            # jump to colour pixel section so we can return
    ...
    colour_pixels_in_display:
        ...
        # The display is 32x32, we want to create an 8x16 grid inside the display
        move $s5, $v1                       # load the base address for display into s5
        ...
        addi $t8, $t8, 104                  # add field offset
        ...
        # viruses
        li $t7, 4
        beq $t5, $t7, set_color_red       # if $t5 is 4, set color to red
        li $t7, 5
        beq $t5, $t7, set_color_green       # if $t5 is 5, set color to green
        li $t7, 6
        beq $t5, $t7, set_color_blue       # if $t5 is 6, set color to blue
